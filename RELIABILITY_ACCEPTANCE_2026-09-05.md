...
## 按钮无响应与后台核对覆盖（2026-09-05）

### 已复现的根因

正式页面没有全局 JavaScript 异常；模式切换和矩阵选择可由真实交互立即生效。但背景 `loadLibrary()` 会把 Album 架直接重建为曲库记录。旧逻辑只有在 `sync.base` 为空时才把工作区中缺失的已完成 Album 补入曲库；存在历史基线时，背景核对会把较短曲库快照覆盖到页面。正式页面在复现期间的 Album 计数由 17 变为 16，证明存在“点击后页面看起来回退”的风险；现有快照不足以断言哪一张为权威版本，未自动恢复或写远端。

### 修复与验收

- 每次背景核对都先把仅在工作区的完成 Album 加入本地 canonical library，标记为待用户确认同步；远端缺失该 Album 时三方合并保留本地版本。
- 恢复发生时页面明确提示保留数量，避免静默改变或误以为按钮失效。
- “导出本地备份 / 冲突明细”使用挂载到文档的下载链接并显示“本地恢复包已开始下载”，修复嵌入式浏览器对未挂载链接可能静默忽略的问题。
- 隔离 React 浏览器回归构造“工作区 2 张 Album、曲库 1 张且已有基线”的场景；刷新后仍为 2 张，并确认导出文件名、可见反馈、零业务远程写入。Node 数据回归 25/25 通过。

一次正式页面验证中按旧无障碍索引误触“重试曲库同步”。当时已有 22 项冲突；代码先完成只读 GET/reconcile，检测到冲突即返回，PUT 在其后，因此未向 `main` 写入。页面最终仍显示 22 项待核对冲突。


## Album 状态按钮不变化（2026-09-05）

截图中的竖排文字是状态按钮的说明气泡，不是下拉菜单。状态切换顺序为“草稿 → 已采用 → 排队中 → 已发布 → 舍弃”。

已复现根因：`setAlbumStatus` 生成新状态后进入 `persistCanonicalAlbum`，而 canonical library 保存逻辑又用尚未重渲染的旧 Album 架状态覆盖该记录，因此点击后视觉上仍是原状态。

修复把人工状态选择作为本次本地提交的最新状态覆盖项，只有主记录成功写入后才更新卡片；浏览器存储失败时保持旧状态并显示错误，不会伪装为成功。隔离浏览器实际点击“排队中”后变为“已发布”，刷新后仍是“已发布”；在注入 quota 写失败后主记录字节与旧状态保持。Node 完整回归 39/39、服务与启动检查通过。正式历史状态未改动，因 22 项冲突尚未裁决。
